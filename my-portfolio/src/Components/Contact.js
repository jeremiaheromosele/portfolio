import React from 'react'

const Contact = () => {
  return (
    <section name='contact' className='pt-20 w-full h-full bg-gradient-to-b from-black to-gray-800 p-4 text-white'>
        <div className='flex flex-col p-4 justify-center max-w-screen-lg mx-auto h-full '>
            <div className='pb-8'>
                <p className='text-4xl font-bold inline border-b-4 border-yellow-500'>Contact</p>
                <p className='py-6'>Submit a response to get in touch we me</p>
            </div>
            <div className='flex justify-center items-center'>
                <form action='https://getform.io/f/4bd6c9fa-0406-474c-acd6-5cd7b8eca048' className=' flex flex-col w-full md:w-1/2 ' method='post'>
                    <input type='text' name='name' placeholder='Enter your name' className='p-2 bg-transparent border-2 rounded-md text-white focus:outline-none'/>
                    <input type='text' name='email' placeholder='Enter your email' className='p-2 bg-transparent my-4 border-2 rounded-md text-white focus:outline-none'/>
                    <textarea name='massage' rows={7} placeholder='Enter your massage' className='p-2 bg-transparent border-2 rounded-md text-white focus:outline-none'></textarea>
                    <button className='text-white bg-gradient-to-b from-cyan-500 to-blue-500 px-6 py-3 my-4 mx-auto flex items-center rounded-md hover:scale-110'>Let's connect</button>
                </form>
            </div>
        </div>
    </section>
  )
}

export default Contact